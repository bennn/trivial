...
(require
  ...
  trivial/private/db/mysql
  ; trivial/private/db/postgres ;; TODO
...
(define (schema-parser stx)
  ...
  (cond
   [(and (list? x*)
    ...
    (list x*)]
   [(and (list? x*)
         (andmap table-parser? x*))
    x*]
   [else #f]))

(define (table-parser? x*)
  (and (list? x*)
       (= 2 (length x*))
       (symbol? (car x*))
       (list? (cadr x*))
...
(define (condition*->type* schema condition* #:src stx)
  ...
  (define unsorted
    (for/fold ([acc '()])
      ...
      (define varnum
        (if (mysql-parameter? val)
          (let ((nn (unbox *mysql-varnum)))
            (set-box! *mysql-varnum (+ 1 nn))
            nn)
          #false))
      (if varnum
        (cons (cons typ varnum) acc)
        acc)))
  (for/list ([typ+num (sort unsorted < #:key cdr)]
             [num (in-naturals 1)])
